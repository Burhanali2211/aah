...
export const db = {
  ...
  async getCategories() {
    ...
    if (error) throw error;
    // Supabase returns products as [{count: N}] when using aggregation
    return (data || []).map((cat: any) => {
      const productsField = cat.products;
      const count = Array.isArray(productsField) && productsField.length > 0
        ? (productsField[0]?.count ?? productsField.length)
        : (cat.product_count || 0);
