  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formData.isDefault) {
        // Unset other defaults before inserting/updating
        await supabase
          .from('addresses')
          .update({ is_default: false })
          .eq('user_id', user.id)
      ...
      if (editingAddress) {
        const { error } = await supabase.from('addresses').update(dbData).eq('id', editingAddress.id);
        if (error) throw error;
        showSuccess('Success', 'Address updated successfully');
      } else {
        const { error } = await supabase.from('addresses').insert(dbData);
        if (error) throw error;
