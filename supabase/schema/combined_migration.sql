...
--   AA   = site prefix (AligarhAttarHouse)
...
VALUES
  -- Branding
  ('site_name',            '"AligarhAttarHouse"',                              'branding',  true,  'The public name of the store'),
  ('site_tagline',         '"Pure Attars from the Heart of AligarhAttarHouse"',       'branding',  true,  'Shown below the site name on the homepage'),
  ...
  -- Contact
  ('contact_email',        '"info@AligarhAttarHouse.com"',                       'contact',   true,  'Public contact email address'),
  ('contact_phone',        '"+91 00000 00000"',                             'contact',   true,  'Public phone number'),
  ('contact_address',      '"AligarhAttarHouse, Uttar Pradesh, India"',               'contact',   true,  'Business address shown on contact page'),
  ('support_email',        '"support@AligarhAttarHouse.com"',                    'contact',   false, 'Internal support email (not shown publicly)'),

  -- SEO
  ('meta_title',           '"AligarhAttarHouse – Authentic Indian Perfumes"',  'seo',       true,  'Default HTML <title> tag'),
  ('meta_description',     '"Shop pure, authentic attars and perfumes crafted in the tradition of AligarhAttarHouse."', 'seo', true, 'Default meta description'),
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.contact_information LIMIT 1) THEN
    ...
    VALUES
      ('email',   'General Enquiries', 'info@AligarhAttarHouse.com',      true,  true, 1),
      ('email',   'Support',           'support@AligarhAttarHouse.com',   false, true, 2),
      ('phone',   'WhatsApp / Call',   '+91 00000 00000',            true,  true, 3),
      ('address', 'Store Address',     'AligarhAttarHouse, Uttar Pradesh, India', true, true, 4);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.social_media_accounts LIMIT 1) THEN
    ...
    VALUES
      ('instagram', 'Instagram', 'https://instagram.com/AligarhAttarHouse', true,  1),
      ('facebook',  'Facebook',  'https://facebook.com/AligarhAttarHouse',  true,  2),
      ('whatsapp',  'WhatsApp',  'https://wa.me/910000000000',         true,  3),
      ('youtube',   'YouTube',   'https://youtube.com/@AligarhAttarHouse',  false, 4);
  END IF;
END $$;
